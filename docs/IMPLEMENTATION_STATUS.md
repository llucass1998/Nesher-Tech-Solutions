...
- LogiDesk web removeu server actions de notificacoes/configuracoes; essas mutacoes agora rodam em Client Components e usam o helper REST com Authorization/refresh. Server actions de tickets e leituras SSR ainda precisam migrar.
...
| Outbox/Redis/retry | Fundacao de outbox, Redis e workers existe | Nao | Parcial | PASS para build/health/worker ready, backoff progressivo, espelho Redis Streams e API inicial de DLQ nos dois lados; consumidores stream-first e reprocessamento fim a fim pendentes |
...
| `npm run build -w logidesk-web` | PASS | Next build passou com `/login`, refresh/logout client-side, helper REST autenticado, `RealtimeNotifications` e `/tickets/[id]`; aviso conhecido de root/lockfiles permanece. |
