...
| LogiFlow and LogiDesk dispatch | Partially implemented | `logiflow-worker` dispatches LogiFlow outbox records to LogiDesk. `logidesk-worker` dispatches ticket events with LogiFlow references back to LogiFlow. Polling-based progressive backoff and initial LogiDesk DLQ administrative reprocessing exist; Redis Streams, LogiFlow DLQ administration and E2E remain pending. |
