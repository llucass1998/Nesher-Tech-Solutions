...
| LogiFlow and LogiDesk dispatch | Partially implemented | `logiflow-worker` dispatches LogiFlow outbox records to LogiDesk. `logidesk-worker` dispatches ticket events with LogiFlow references back to LogiFlow. Polling-based progressive backoff, Redis Streams mirrors and initial DLQ administrative reprocessing exist on both APIs; stream-first consumers and E2E remain pending. |
