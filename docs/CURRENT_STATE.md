...
- O helper REST do web ja envia Authorization no browser. Notificacoes, configuracoes e acoes do detalhe do ticket ja usam mutacoes client-side autenticadas, e server actions antigas de tickets foram removidas. A API possui RBAC REST de transicao para mutacoes quando `LOGIDESK_REQUIRE_REST_AUTH=true`; ainda falta migrar leituras SSR para sessao autenticada, ligar a flag por padrao, aplicar ownership fino e completar processamento dos workers.
