...
export class LogiPeopleOutboxDispatcher {
  constructor(
  ...
  async dispatchPendingOutboxBatch() {
    if (!this.pool) {
      this.logger.warn({ operation: 'outbox.dispatch', status: 'skipped', reason: 'LOGIPEOPLE_DATABASE_URL missing' }, 'Outbox dispatch skipped');
      return;
    }
  ...
  async dispatchOutboxEvent(event: OutboxEventRow) {
    try {
      ...
      this.logger.info(
        ...
        'LogiPeople outbox event published',
      );
    } catch (error) {
  ...
  private async markFailed(event: OutboxEventRow, error: unknown) {
    ...
    const retryDelaySeconds = shouldDeadLetter
      ? null
      : calculateRetryDelaySeconds(event.attempts, this.config.retryBaseDelaySeconds, this.config.retryMaxDelaySeconds);
    ...
    this.logger.error(
      ...
      'LogiPeople outbox event dispatch failed',
    );
  }
}
